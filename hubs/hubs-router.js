...
//
// Or, if there isn't a hub_id, it creates ihubt and adds the id.
//
...
// //async way of function syntax
// router.put("/:id", async (req, res) => {
//   //if promise is inside another fn, we must to add async
//   const changes = req.body;
//   try {
//     const hub = await Hubs.update(req.params.id, changes); //in this case
//     //the fn will be put in the event queue and keep app running, and
//     //only when Hubs.update() will get a response, our program will continue here
//     if (hub) {
//       res.status(200).json(hub);
//     } else {
//       res.status(404).json({ message: "The hub could not be found" });
...
// // });
// //----------------------------------------------
// //list out a hub's messages
// router.get("/:id/messages", (req, res) => {
//   Hubs.findById(req.params.id)
//     .then((hub) => {
//       if (!hub) {
//         return res.status(404).json({ message: "Hub not found" }); //we must use return here, since when we send the response, we need to stop executing of this function, we can not send another response.
//         // res.status(200).json(hub);
//       } else {
//         //res.status(404).json({ message: "Hub not found" });
//         return Hubs.findHubMessages(req.params.id);
//       }
//     })
//     .then((messages) => {
//       //chain promise from: return Hubs.findHubMessages(req.params.id)
//       res.status(200).json(messages); //here nothing after this response, that is why we do not use return
//     })
//     .catch((error) => {
//       // log error to database
//       console.log(error);
...
//     });
// });
// //get a single message of a hub by its id    function findMessageById(id)
// router.get("/:id/messages/:messageID", (req, res) => {
//   Hubs.findMessageById(req.params.messageID)
//     .then((message) => {
//       if (message) {
...
//       console.log(error);
//       res.status(500).json({
//         message: "Error getting the hub message",
//       });
//     });
// });
